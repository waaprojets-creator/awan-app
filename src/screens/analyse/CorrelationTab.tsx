...
export function CorrelationTab({ sessions, history, todayKcal }: CorrelationTabProps) {
  const last30 = React.useMemo(() => Array.from({ length: 30 }, (_, i) => {
    ...
    const measure = history.find(m => m.date === str);
  ...
  const latestW = sorted[0]; const oldestW = sorted.at(-1);
