              ...
              <div className="grid grid-cols-5 gap-3 mb-4">
                {theme.palette.map((color, idx) => (
                  <div key={idx} className="flex flex-col items-center gap-2">
                    <div className="w-12 h-12 rounded-lg border border-white/10" style={{ backgroundColor: color }} />
                ...
                {[1.0, 1.1, 1.2, 1.3, 1.4, 1.5].map(v => {
                  ...
                  return (
                    <Touch key={v} onPress={() => updateCfg('jitFactor', v)}
                      className={`flex-1 py-3 items-center justify-center rounded-lg ${active ? 'bg-awan-gold' : 'transparent'}`}>
