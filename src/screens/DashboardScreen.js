...
export default function DashboardScreen({ navigate }) {
  ...
  // Calculs dérivés — protégés contre db null
  ...
  const todayEvents = db ? eventsForDate(db, todayStr).filter(e => e.time) : [];
  ...
  return (
    ...
    >
      ...
      <Card title={L.dash.widgets.tasks} onPress={() => navigate('Tasks')} highlight={tasksLate > 0}>
        <Text style={[s.bigTx, tasksLate > 0 && { color: T.gold }]}>{tasksTotal === 0 ? '0' : tasksTotal}</Text>
        <Text style={s.subTx}>{tasksSubtitle}</Text>
      </Card>

      <Card title={L.dash.widgets.trajet} onPress={() => navigate('Trajet')} demo>
      ...
      <Card title={L.dash.widgets.analyse} onPress={() => navigate('Analyse')}>
      ...
      <Card title={L.dash.widgets.macros} onPress={() => navigate('Sante')} demo>
        {fakeMacros.map(m => (
          <View key={m.key} style={s.macroRow}>
            <Text style={s.macroLbl}>{m.label}</Text>
            <View style={s.macroBarBg}>
              <View style={[s.macroBarFg, { width: `${Math.min(100, (m.current / m.goal) * 100)}%`, backgroundColor: m.color }]} />
            </View>
      ...
      <Card title={L.dash.widgets.sport} onPress={() => navigate('Sante')} demo>
      ...
      <Card title={L.dash.widgets.courses} onPress={() => navigate('Sante')} demo>
      ...
      <Card title={L.dash.widgets.week} onPress={() => navigate('Analyse')} demo>
  ...
  return (
    <Svg width={size} height={size}>
      {arcs.map((a, i) => {
        ...
        return (
          <Path key={i} d={`M ${cx} ${cy} L ${x1} ${y1} A ${r} ${r} 0 ${large} 1 ${x2} ${y2} Z`} fill={a.color}/>
        );
      })}
...
const s = StyleSheet.create({
  ...
  dateBanner: { fontSize: 11, color: T.tx2, letterSpacing: 2, fontWeight: '600', textAlign: 'center', marginVertical: 12 },
  card: { backgroundColor: T.bg2, borderWidth: 1, borderColor: T.bo, borderRadius: 14, padding: 16, marginBottom: 12 },
  ...
  transportBtn: { flex: 1, alignItems: 'center', paddingVertical: 10, paddingHorizontal: 4, borderRadius: 10, borderWidth: 1, borderColor: T.bo, backgroundColor: T.bg },
